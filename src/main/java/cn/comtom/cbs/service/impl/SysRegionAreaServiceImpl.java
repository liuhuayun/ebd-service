package cn.comtom.cbs.service.impl;

import cn.comtom.cbs.dao.SysRegionAreaMapper;
import cn.comtom.cbs.model.SysRegionArea;
import cn.comtom.cbs.service.SysRegionAreaService;
import cn.comtom.cbs.core.AbstractService;
import org.springframework.stereotype.Service;
import org.springframework.transaction.annotation.Transactional;

import javax.annotation.Resource;


/**
 * SysRegionArea Service实现类，Created on 2019-06-19.
 * @author William
 */
@Service
@Transactional(rollbackFor = Exception.class)
public class SysRegionAreaServiceImpl extends AbstractService<SysRegionArea> implements SysRegionAreaService {
    @Resource
    private SysRegionAreaMapper sysRegionAreaMapper;

}
