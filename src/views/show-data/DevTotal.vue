<template>
  <div>
    <!-- 面包屑导航 -->
    <el-breadcrumb separator-class="el-icon-arrow-right">
      <el-breadcrumb-item>设备统计数据</el-breadcrumb-item>
    </el-breadcrumb>
    <!-- 搜索条件 -->
    <user-card :expand="false">
       <el-select :default-active="$route.path" @change="chickvalue" v-model="searchValue" filterable placeholder="请选择汇总维度" class="h-m-select">
          <el-option
            v-for="item in options"
            :key="item.value"
            :label="item.label"
            v-model="item.value">
          </el-option>
        </el-select>
        <el-button type="primary" size="small" @click.native='Count'>统计
        </el-button>
    </user-card>
    <!-- 表格 -->
    <el-table :data="get_dev_total.list"
      border
      style="width: 100%">
      <el-table-column prop="onlineStatus"
        label="onlineStatus"
        align="center">
      </el-table-column>
      <el-table-column prop="bizStatus"
        label="bizStatus"
        align="center">
      </el-table-column>
      <el-table-column prop="total"
        label="total"
        align="center">
      </el-table-column>
      <el-table-column prop="bindDate"
        label="bindDate"
        align="center">
      </el-table-column>
      <el-table-column prop="count"
        label="count"
        width="120"
        align="center">
      </el-table-column>

    </el-table>
    <!-- 分页 -->
    <div class="pagination">
      <el-pagination background
        layout="total, prev, pager, next"
        @current-change="handleCurrentChange"
        :total="get_dev_total.total">
      </el-pagination>
    </div>
  </div>
</template>
<script>
import { mapGetters, mapActions } from 'vuex';
import UserCard from '../../components/UserCard';

export default {
  name: 'DevTotal',
  data() {
    return {
      options: [{
        value: 'type',
        label: '类型',
      }, {
        value: 'status',
        label: 'status',
      }],
      searchValue: '',

    };
  },
  components: {
    UserCard,
  },
  computed: {
    ...mapGetters(['get_dev_total']),
  },
  methods: {
    ...mapActions(['getDevTotal']),
    handleCurrentChange(val) {
      this.getDevTotal({
        params: {
          page: val,
          size: 10,
        },
      });
    },
    Count() {
      this.getDevTotal({
        params: {
          page: 1,
          size: 10,
          sumType: this.searchValue,
        },
      });
    },
  },
  mounted() {
    this.getDevTotal({
      params: {
        page: 1,
        size: 10,
      },
    });
  },
};
</script>
<style lang="less" scoped>
</style>
